var conf_livingDoc = new function()
{
	...
	this.registerSpecification=function(bUID, exeUID, ctx, imgFile, spaceKey, pageId, fieldId, maxResponseTime, actions, isMain)
	{
		var bulk = this.getBulk(bUID);
		if(!bulk){ bulk = this.registerBulk(bUID, actions); }
		...
		return list.registerSpecification({spaceKey:spaceKey, pageId:pageId, fieldId:fieldId,maxResponseTime:maxResponseTime, isMain:isMain?true:false});
	};	
